...
class WorkerDao extends BaseDao implements BaseDaoInterface {
    ...
    public function insert($item) {
        ...
        $data = array(
            ...
            "wor_isuprectime" => $item["worIsuprectime"],
        );
    ...
    public function queryListByCondition($cond, $page = 0, $count = 100) {
        ...
        return M("Worker")->alias('wor')->field('wor_code,wor_name,wor_rname,wor_auth,wor_token,wor_phone,wor_isuprectime,wor.war_code,war_name,war_address,war_postal,war_info,rooms')
            ->join("JOIN wms_warehouse war ON war.war_code = wor.war_code")
    ...
    public function updateItemByCode($code, $item) {
        ...
        if (isset($item["worIsuprectime"])) $data["wor_isuprectime"] = $item["worIsuprectime"];
    ...
    //更新大仓user表的小程序入仓选择时间权限
    public function updateWmsUserByToken($worToken, $worIsuprectime) {
        $condition = array( "user_token" => $worToken);
        return M("zwdb_wms.user")->where($condition)
            ->save(array("user_isuprectime" => $worIsuprectime, "timestamp" => venus_current_datetime()));
    }
}