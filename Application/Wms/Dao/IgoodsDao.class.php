...
class IgoodsDao extends BaseDao implements BaseDaoInterface
{
    ...
    public function queryByCode($code)
    {
        ...
        return M("Igoods")->alias('igo')->field('*,spu.spu_code,sku.sku_code,igo.sku_count')
    ...
    public function updateCountAndSkuCountByCode($code, $count, $skucount)
    {
        $condition = array("igo.war_code" => $this->warehousecode, "igo_code" => $code);
        return M("Igoods")->alias('igo')
            ->where($condition)->fetchSql(false)->save(array("igo_count" => $count, "sku_count" => $skucount));
    }

    public function queryByInvCodeAndSpuCodeAndCount($invCode,$spuCode, $count)
    {
        $condition = array("igo.war_code" => $this->warehousecode, "inv_code" => $invCode,"igo.spu_code"=>$spuCode,"igo.igo_count"=>$count);
        return M("Igoods")->alias('igo')->field('*,spu.spu_code,sku.sku_code')
            ->join("LEFT JOIN wms_sku sku ON sku.sku_code = igo.sku_code AND sku.spu_code={$spuCode}")
            ->join("LEFT JOIN wms_spu spu ON spu.spu_code = sku.spu_code AND sku.spu_code={$spuCode}")
            ->where($condition)->order('igo.igo_code')->fetchSql(false)->find();
    }
}