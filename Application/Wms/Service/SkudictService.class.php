    ...
    public function valid_and_create_skudict($skudictData)
    {
        ...
        if (!empty($skudictData)) {
            ...
            if (empty($skuDictresult)) {
                ...
                if ($skudictInsert) {
                    $success = $success && $skudictInsert;
                } else {
                    $success = $success && $skudictInsert;
                }
            }

            $supDao = SupplierDao::getInstance();
            $supCtesult = $supDao->queryAllByCode($skudictData['supCode']);
            if (empty($supCtesult)) {
                $supData = array(
                    "code" => $skudictData['supCode'],
                    "name" => $skudictData['supName'],
                    "mark" => '',
                    "phone" => $skudictData['supPhone'],
                    "manager" => $skudictData['supManager'],
                );
                $supInsert = $supDao->supInsert($supData);
                if ($supInsert) {
                    $success = $success && $supInsert;
                } else {
                    $success = $success && $supInsert;
                }
            }
