...
foreach ($skuInfoData as $skuInfoDatum) {
    if (substr($skuInfoDatum['sku_code'], 0, 2) == "SK") {
        $data['数据'][] = array(
...
if (!empty($fileName)) {
    ...
    $newFname = "minisku.xlsx";
    $moveFileRes = rename("/home/dev/venus-mini/Public/files/002/$fileName", "/home/dev/venus-mini/Public/files/sku/database/$newFname");
    if ($moveFileRes) {
        echo $newFname;
    } else {
        echo "移动失败" . "$newFname";
