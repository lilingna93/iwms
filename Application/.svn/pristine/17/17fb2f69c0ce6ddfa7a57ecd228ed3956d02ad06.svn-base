...
{
    E(C("EXCEPTION")[$error] . (isset($description) ? ":" : "") . $description, $error);
}
...
{
//    return "2018-10-29".date(" H:i:s", time());
...
//获取SPU的报表分类名称
function venus_spu_reptype_name($typecode)
{
    return C("SPU_REPORT_TYPE")[$typecode];
}
...
//入仓单类型说明
function venus_receipt_type_desc($status)
{
    return C("RECEIPT_TYPE")[$status];
}
 ...
 * 计算总价
 */
function venus_calculate_sku_price_by_spu($sprice, $count, $percent)
{
    $profit = bcmul($sprice, $percent, 6);
    $totalprofit = bcmul($profit, $count, 6);
    $totalsprice = bcmul($sprice, $count, 6);
    $totalprice = round(bcadd($totalsprice, $totalprofit, 6), 2);
    return $totalprice;
}
 ...
 * @return bool
 */
function sendMailer($title, $content)
{
